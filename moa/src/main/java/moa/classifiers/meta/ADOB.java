...
public class ADOB extends AbstractClassifier implements MultiClassClassifier {
    ...
    public FloatOption lambdaOption = new FloatOption("lambda", 'a', "Lambda for down sampling.",
            1.0);
	...
	// Sort by accuracy in ascending order
        ...
        //double lambda_d = 1.0;
        // Shujie add:
        double lambda_d = lambdaOption.getValue();
        ...
        for (int i = 0; i < this.ensemble.length; i++) {
            ...
            if (inst.classValue() == 0) {
                k = this.pureBoostOption.isSet()? lambda_d/this.downSampleRatio: MiscUtils.poisson(
                        lambda_d / this.downSampleRatio, this.classifierRandom);
            } else {
                k = this.pureBoostOption.isSet()? lambda_d : MiscUtils.poisson(lambda_d, this.classifierRandom);
            }
            if (k > 0.0) {
